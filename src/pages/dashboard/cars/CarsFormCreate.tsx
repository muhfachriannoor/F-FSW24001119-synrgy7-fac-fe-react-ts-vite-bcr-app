...
import { useCreateCars } from "@/features/cars/useCreateCars";
import LayoutDashboard from "@/components/dashboard/Layout";
import SidebarSubMenu from "@/components/dashboard/SidebarSubMenu";
import Alert from "react-bootstrap/Alert";
import Swal from "sweetalert2";

export default function CarsFormCreate(): ReactElement {
  const { register, handleSubmit } = useForm<IFormCars>();
  const [isError, setIsError] = useState<boolean>(false);
  const [errorMessage, setErrorMessage] = useState<string>("");
  const [errorData, setErrorData] = useState<string[]>([]);
  const navigate = useNavigate();

  const { mutate, isPending } = useCreateCars({
    onSuccess: () => {
      Swal.fire({
        title: "Success Add Car!",
        icon: "success",
      }).then(() => {
    ...
    onError: (error: any) => {
      ...
      } else if (error.message.response.data.message == "You cannot access this feature!") {
        Swal.fire({
